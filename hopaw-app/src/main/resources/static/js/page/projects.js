...
var notifyChannelsCache = [];    // 通知渠道缓存（供项目表单多选渲染）
var notifyChannelEditing = null; // 渠道管理弹窗当前编辑的渠道（null=新增）
...
function showAddModal() {
    ...
    // 通知渠道与事项：新建默认不勾选，加载最新渠道列表
    loadNotifyChannels([]);
    renderNotifyEventCheckboxes([]);
...
function editCurrentProject() {
    ...
    // 回填通知渠道与事项勾选（加载最新渠道列表后按项目配置勾选）
    loadNotifyChannels(project.notifyChannelIds || []);
    renderNotifyEventCheckboxes(project.notifyEventCodes || []);
    Modal.open('projectModal');
}
...
/**
 * 渲染项目表单的通知渠道多选框（保留当前已勾选状态，渠道缓存变化后重渲染）
 * @param selectedIds 勾选的渠道编号数组（不传时保留现有勾选）
 */
function renderNotifyChannelCheckboxes(selectedIds) {
    var box = document.getElementById('projectNotifyChannelGroup');
    if (!box) return;
    var keep = selectedIds || getCheckedNotifyValues('projectNotifyChannelGroup');
    var html = '';
    if (!notifyChannelsCache.length) {
        html = '<small class="form-hint">暂无通知渠道，点击上方「管理渠道」新增</small>';
    }
    notifyChannelsCache.forEach(function (ch) {
        var checked = keep.indexOf(ch.id) !== -1 ? ' checked' : '';
        var typeLabel = NOTIFY_CHANNEL_TYPES[ch.type] || ch.type;
        html += '<label class="checkbox-label"><input type="checkbox" value="' + ch.id + '"' + checked + '> ' +
            escapeHtml(ch.name) + '（' + typeLabel + (ch.enabled === false ? '，已停用' : '') + '）</label>';
    });
    box.innerHTML = html;
}
...
/**
 * 渲染项目表单的通知事项多选框
 * @param selectedCodes 勾选的事项编码数组（不传时保留现有勾选）
 */
function renderNotifyEventCheckboxes(selectedCodes) {
...
/** 读取多选框容器内全部勾选值 */
function getCheckedNotifyValues(groupId) {
    ...
    box.querySelectorAll('input[type="checkbox"]:checked').forEach(function (cb) {
        var v = cb.value;
        // 渠道编号为数值
        values.push(/^\d+$/.test(v) ? Number(v) : v);
    });
    return values;
}
...
function openNotifyChannelModal() {
    ...
    Modal.open('notifyChannelModal');
}

/** 关闭渠道管理弹窗 */
function closeNotifyChannelModal() {
    Modal.close('notifyChannelModal');
}
...
function onNotifyChannelTypeChange() {
    ...
    if (type === 'dingtalk') {
        ...
        html = notifyCfgInput('receivers', '收件邮箱 *', '多个邮箱用逗号分隔，如 a@b.com,c@d.com；SMTP 服务器在系统设置中配置');
    } else {
...
function notifyCfgInput(key, label, placeholder) {
    return '<div class="form-group"><label>' + label + '</label>' +
        '<input type="text" data-cfg="' + key + '" placeholder="' + escapeHtml(placeholder) + '"></div>';
}
        ...
        .then(function (res) {
            if (res.code === 200) {
                ...
                fetch('/api/notify/channels')
                    .then(function (r) { return r.json(); })
                    .then(function (r2) {
                        notifyChannelsCache = r2.code === 200 ? (r2.data || []) : [];
                        renderNotifyChannelList();
                        renderNotifyChannelCheckboxes();
                    });
            } else {

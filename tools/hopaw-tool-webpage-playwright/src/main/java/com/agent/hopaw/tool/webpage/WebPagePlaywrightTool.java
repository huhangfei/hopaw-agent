...
public class WebPagePlaywrightTool extends AbstractAgentTool {
    ...
    @Tool(name = "webPagePlaywright_fetch", value = {"获取网页用Playwright", "获取网页内容，输入URL地址，返回网页的纯文本或HTML源文件"})
    ...
    @Tool(name = "webPagePlaywright_screenshot", value = {"网页截图", "使用Playwright截取指定URL网页的整页图片并保存到本地文件。支持png/jpg/jpeg格式，按保存路径扩展名自动选择格式，自动创建父目录。", "网页,截图,截图保存,页面截图"})

...
public class ProjectIterateService implements IProjectIterateService {
    ...
    private IAgentExecutor createProjectExecutor(String requestId,Project project, Agent agent, String sessionId,
        ...
        params.setMaxToolInvocations(Agent.resolveMaxToolInvocations(agent.getMaxToolInvocations()));
    ...
    private String buildProjectSystemMessage(Project project, Agent agent) {
        ...
        sb.append("8.记忆工具是你的核心工具，需要回忆什么信息时，先去调用记忆工具看看有没相关可用信息，只能查询记忆不要添加修改记忆。\n");
        sb.append("9.你只能使用用户提供的工具，绝对不能调用不存在的工具，遇到危险操作立刻停止。\n");

        sb.append("\n--- 常规项目迭代工作流程 ---\n");
        sb.append("1.分析项目目标与当前任务进度，判断项目目标是否需要拆解为更多任务；\n");
        sb.append("2.对待启动的任务进行审核：确认任务内容合理后审核通过，使其进入执行；\n");
        sb.append("3.对待验收的任务进行检查,注意要检查交付物是否符合要求：验收合格则通过，不合格则驳回并说明原因；\n");
        sb.append("4.对失败的任务视情况执行重做或调整任务内容再执行重做；对确认不再需要执行的任务及时关闭（不要删除任务）；\n");

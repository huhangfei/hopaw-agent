...
Java 17 + Spring Boot 2.7.18 + LangChain4j 1.19.0 AI Agent platform. Multi-module Maven project with SQLite backend.
...
**Active plugin modules** (in `tools/`): `ssh`, `demo`, `dingtalk`, `baidusearch`, `file`, `command`, `database`.
`hopaw-tool-webpage` is commented out in root `pom.xml` and not built.
...
Full build → image → run workflow:

```bash
maven-package.bat                    # Step 1: build all modules + collect deps into docker-libs/
docker-build.bat                     # Step 2: build image hopaw-agent:1.0.0
docker-compose up -d                 # Step 3: run (data persisted in ./data/)
```

Single-module hot update (no image rebuild): copy the module JAR to `data/lib-override/`, restart container.

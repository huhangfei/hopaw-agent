...
window.onload = function() {
    ...
    // 输入工具栏窄时收起带文字按钮的文字，仅保留图标（模型/技能/调用权限），避免图标+文字排不下
    var inputTools = document.querySelector('.chat-input-tools');
    if (inputTools && typeof ResizeObserver !== 'undefined') {
        var syncInputToolbarCompact = function() {
            inputTools.classList.toggle('compact', inputTools.clientWidth < 600);
        };
        new ResizeObserver(syncInputToolbarCompact).observe(inputTools);
        syncInputToolbarCompact();
    }
};
...
function updateSkillBtnState() {
    ...
    if (selected.length > 0) {
        ...
        btn.innerHTML = svg + '<span class="btn-label">技能 (' + selected.length + ')</span>';
    } else {
        btn.classList.remove('has-selected');
        var svg = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>';
        btn.innerHTML = svg + '<span class="btn-label">技能</span>';
    }
}

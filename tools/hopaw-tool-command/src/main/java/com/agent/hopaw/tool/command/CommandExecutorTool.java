...
public class CommandExecutorTool extends AbstractAgentTool {
    ...
    @Tool(name = "command_getOsName", value = {"获取操作系统名称", "获取本地操作系统的名称，例如 Windows 10 或 Ubuntu 20.04"})
    public String getOsName() {
        return System.getProperty("os.name");
    }

    @ToolSecurityLevel(ToolSecurityLevel.Level.ALL_REQUIRE_APPROVAL)
    @Tool(name = "command_execute", value = {"执行命令", "执行本地系统命令并返回输出结果。支持 Windows 和 Unix/Linux/macOS 系统。使用前最好先获取操作系统类型，" +
            "以确保命令在目标系统上执行。" +

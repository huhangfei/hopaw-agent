...
public class AvatarTool implements AgentTool {
    ...
    @Tool(name = "avatar_changeModel", value = {"虚拟人换装", "让虚拟人换装：从用户可用的模型池中随机抽取一个不同的模型进行切换"},
    ...
    @ToolSecurityLevel(ToolSecurityLevel.Level.SAFE)
    @Tool(name = "avatar_move", value = {"虚拟人走动", "控制虚拟人在用户屏幕上的相对移动（以当前所在位置为原点(0,0)，传入目标坐标和移动时长）"},
            searchBehavior = SearchBehavior.ALWAYS_VISIBLE)
    ...
    @ToolSecurityLevel(ToolSecurityLevel.Level.SAFE)
    @Tool(name = "avatar_sendMessage", value = {"虚拟人消息", "向用户发送虚拟人主动消息，该消息支持文字气泡显示和语音播报"},
